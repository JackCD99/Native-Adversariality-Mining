...
NAM treats the **initial noise of a pretrained diffusion model as the search space**.  
Instead of modifying the diffusion model or injecting adversarial guidance throughout the denoising trajectory, NAM learns a lightweight **adversariality miner** that identifies noise regions more likely to generate informative hard samples.

During miner training, both the diffusion generator and the downstream model are frozen. Only the NAM miner is optimized. After training, the miner is used to reselect the initial noise, while the original diffusion sampler remains unchanged.

### 🔄 Overall pipeline

```text
┌─────────────────────────────────────────────────────────────────────────────┐
│                     Native Adversariality Mining (NAM)                     │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   Real training data                                                        │
│        │                                                                    │
...
│        │                      │                                              │
│        │                      ▼                                              │
│        │               Frozen downstream anchor                            │
│        │                      │                                              │
│        │                      │ evaluates synthetic difficulty               │
│        │                      ▼                                              │
│   Condition / prompt ─► Frozen diffusion model ◄──── Initial noise z ~ N(0,I)
│                              │                         │                     │
│                              │ initial response        │                     │
│                              ▼                         │                     │
│                        Initial denoising score         │                     │
│                              │                         │                     │
│                              ▼                         │                     │
│                           NAM miner                    │                     │
│                              │                         │                     │
│                    predicts noise-prior shift          │                     │
│                              │                         │                     │
│                              ▼                         │                     │
│                       Mined initial noise z_adv ◄──────┘                     │
│                              │                                              │
│                              ▼                                              │
│                     Frozen diffusion sampler                               │
│                              │                                              │
│                              ▼                                              │
...
### ⛏️ Algorithm 1 — Training the NAM miner

```text
Algorithm: TRAIN_NAM_MINER

Inputs
    D_real          real training dataset
    G_theta         pretrained diffusion generator
    F_phi           downstream model trained on real data
    M_xi            NAM adversariality miner

    T               diffusion sampling horizon
    K               number of truncated differentiable denoising steps
    beta            KL regularization weight
    kappa_up        upper control for adversariality optimization
    N_iter          number of NAM optimization iterations

    L_adv           adversariality objective
                    e.g. LCE / LCBCE / LDice / LFocal

Output
    M_xi*           trained NAM miner
...
1:  Freeze G_theta
2:  Freeze F_phi
3:  Initialize parameters xi of M_xi

4:  for iteration = 1, ..., N_iter do

5:      # ---------------------------------------------------------------
6:      # A. Sample a real condition
7:      # ---------------------------------------------------------------

8:      Sample a training condition c from D_real
        ...
        Examples:
            ...
            text-to-image:
                c = class-aware text prompt

            joint image-mask generation:
                c = task-specific conditioning information


9:      # ---------------------------------------------------------------
10:     # B. Draw ordinary diffusion noise
11:     # ---------------------------------------------------------------

12:     Sample epsilon ~ N(0, I)

13:     Set initial diffusion state

            x_T = epsilon


14:     # ---------------------------------------------------------------
15:     # C. Probe the frozen diffusion model
16:     # ---------------------------------------------------------------

17:     Run only the initial diffusion response

            s_T = G_theta(x_T, T, c)

18:     where s_T is the initial denoising / score response produced by
        the frozen generator.

19:     No diffusion parameter is updated.


20:     # ---------------------------------------------------------------
21:     # D. Predict an adversarial noise distribution
22:     # ---------------------------------------------------------------

23:     Feed the initial diffusion response into the NAM miner

            Delta_mu, Delta_Sigma = M_xi(s_T)

24:     Construct the mined noise distribution
...
25:     Constrain the predicted variance shift to its allowed range.

26:     Reparameterize a mined initial noise

            eta ~ N(0, I)

            z_adv =
                Delta_mu
                + sqrt(I + Delta_Sigma) ⊙ eta


27:     # ---------------------------------------------------------------
28:     # E. Perform a truncated diffusion rollout
29:     # ---------------------------------------------------------------

30:     Start the frozen diffusion sampler from z_adv
...
35:     Obtain an intermediate synthetic prediction

            x_hat^(K)

36:     Gradients are retained only through the truncated rollout required
        for NAM optimization.

37:     The remaining diffusion trajectory is not unfolded during every
        miner update.


38:     # ---------------------------------------------------------------
39:     # F. Measure downstream adversariality
40:     # ---------------------------------------------------------------

41:     Feed the synthetic prediction to the frozen downstream anchor

            y_hat = F_phi(x_hat^(K))

42:     Compute the configured adversariality objective
...
43:     Interpret A as the difficulty of the generated sample for the
        frozen downstream model.

44:     Larger adversariality
            =>
        harder synthetic sample.


45:     # ---------------------------------------------------------------
46:     # G. Control excessive adversariality
47:     # ---------------------------------------------------------------

48:     Apply the configured adversariality control using kappa_up

            A_controlled =
                ADVERSARIALITY_CONTROL(
                    A,
                    upper_bound = kappa_up
                )

49:     This prevents the miner from obtaining arbitrarily large reward
        simply by moving toward pathological or unstable regions.


50:     # ---------------------------------------------------------------
51:     # H. Preserve proximity to the native diffusion prior
52:     # ---------------------------------------------------------------

53:     Compute the divergence between the mined prior and the original
...
54:     This term discourages unnecessary displacement from the native
        noise distribution.


55:     # ---------------------------------------------------------------
56:     # I. NAM optimization objective
57:     # ---------------------------------------------------------------

58:     The miner should

            maximize    downstream adversariality

        while

            minimizing  departure from the native diffusion prior.

59:     Equivalently, minimize
...
60:     # ---------------------------------------------------------------
61:     # J. Update ONLY the miner
62:     # ---------------------------------------------------------------

63:     Zero gradients of M_xi

64:     Backpropagate

            grad_xi(L_NAM)

65:     Update xi with the configured optimizer
...
67:     throughout the entire NAM optimization.
...
```text
                    ┌───────────────────────┐
                    │ Original noise prior  │
                    │      N(0, I)          │
                    └──────────┬────────────┘
                               │
                               │ ordinary diffusion seeds
                               ▼
                    ┌───────────────────────┐
                    │    Diffusion model    │
                    │       frozen          │
...
The optimization therefore does **not** search directly in image space and does **not** alter the pretrained diffusion parameters. The learnable object is the mapping from the generator's early denoising response to a shifted initial-noise distribution.

---

### 🧬 Algorithm 2 — Generating samples with a trained NAM miner

After the miner has been trained, synthetic generation uses the **original diffusion sampling procedure**.

```text
Algorithm: GENERATE_WITH_NAM

Inputs
    G_theta         frozen pretrained diffusion generator
    M_xi*           trained NAM miner
    C               conditions / prompts to synthesize
    B               requested synthetic-data budget
    S               diffusion sampling schedule

Output
    D_NAM           NAM-generated synthetic dataset
...
5:      Select condition c from C


6:      # ---------------------------------------------------------------
7:      # A. Sample an ordinary Gaussian probe
8:      # ---------------------------------------------------------------
...
10:     # ---------------------------------------------------------------
11:     # B. Obtain the generator's initial response
12:     # ---------------------------------------------------------------
...
14:     # ---------------------------------------------------------------
15:     # C. Mine the initial noise
16:     # ---------------------------------------------------------------

17:     Delta_mu, Delta_Sigma = M_xi*(s_T)
...
20:     # ---------------------------------------------------------------
21:     # D. Run the ORIGINAL diffusion sampler
22:     # ---------------------------------------------------------------

23:     x <- z_adv

24:     for timestep t in S do
...
27:     x_syn <- DECODE_IF_REQUIRED(x)


28:     # ---------------------------------------------------------------
29:     # E. Save the generated sample
30:     # ---------------------------------------------------------------
...
32:     Add generated record to D_NAM
...
A key distinction between **NAM training** and **NAM sampling** is:

```text
NAM training
    └── truncated differentiable rollout
        └── used to obtain an efficient learning signal for the miner

NAM sampling
    └── full diffusion trajectory
        └── uses the original sampler to produce the final synthetic sample
```

---

### ⚖️ Algorithm 3 — Matched Base vs. NAM generation

The benefit of NAM should be measured under the **same synthetic-data budget and the same generator**.

```text
Algorithm: MATCHED_BASE_VS_NAM_SAMPLING
...
Comparison constraints:
    ...
    same generator checkpoint
    same condition set
    same synthetic-data budget
    same image / volume resolution
    same diffusion sampler
    same number of denoising steps
    same downstream training protocol

Only the initial-noise selection strategy differs.
```

---

### 🩺 Algorithm 4 — Using NAM samples for downstream generalization

NAM itself does not replace downstream training. The mined synthetic samples are added to the downstream training pipeline as augmentation data.

```text
Algorithm: TRAIN_DOWNSTREAM_WITH_NAM

Inputs
    D_real          real training dataset
    D_NAM           NAM-generated synthetic dataset
    F_real          downstream model trained on real data

Output
    F_NAM           synthetic-augmented downstream model


──────────────────────────────────────────────────────────────────────────────

1:  Load the validation-selected real-data checkpoint

        F <- F_real


2:  for each downstream training iteration do

3:      Sample real mini-batch

            B_real ~ D_real

4:      Sample NAM mini-batch

            B_syn ~ D_NAM


5:      Construct the configured real/synthetic training batch
...
6:      Apply the downstream model's standard augmentation policy

7:      Compute the task loss

            segmentation:
                Dice / CE / architecture-specific objective

            classification:
                classification objective


8:      Update downstream parameters

9:  end for


10: Select checkpoint using the validation split

11: Evaluate the selected checkpoint on the held-out test split

12: return F_NAM
```

For the main reproduction protocol, Base and NAM synthetic datasets are evaluated using the **same downstream initialization and continuation schedule** so that improvements can be attributed to the synthetic samples rather than to changes in training budget.

---

### 📊 Algorithm 5 — Measuring synthetic adversariality

The adversariality of a generated sample is evaluated using a frozen downstream model.

```text
Algorithm: EVALUATE_ADVERSARIALITY

Inputs
    D_syn           generated synthetic dataset
    F_phi           frozen downstream model
    L_proxy         selected adversariality proxy

Output
    per-sample adversariality scores
...
2:  for each synthetic sample (x_i, y_i) in D_syn do

3:      prediction_i = F_phi(x_i)
...
5:      Convert the raw task loss into the configured adversariality score

            adv_i = NORMALIZE_ADVERSARIALITY(loss_i)
...
8:  Aggregate
        ...
        mean adversariality
        adversariality distribution
        high-adversariality tail
        Base vs. NAM statistics

9:  return all per-sample and aggregate results
```

The repository supports multiple adversariality proxies:

```text
LCE       Cross-entropy-based difficulty
LCBCE     Class-balanced cross-entropy-based difficulty
LDice     Dice-loss-based difficulty
LFocal    Focal-loss-based difficulty
```

Different proxies can be selected through the experiment configuration.
...
```text
┌─────────────────────────────┬────────────────────┬──────────────────────────┐
│ Component                   │ NAM Training       │ NAM Sampling             │
├─────────────────────────────┼────────────────────┼──────────────────────────┤
│ Diffusion generator         │ Frozen             │ Frozen                   │
│ Downstream anchor           │ Frozen             │ Not required for basic   │
│                             │                    │ NAM generation           │
│ NAM miner                   │ Trainable          │ Frozen                   │
│ Initial random noise        │ Yes                │ Yes                      │
│ Initial diffusion probe     │ Yes                │ Yes                      │
│ Noise-prior transformation  │ Yes                │ Yes                      │
│ Differentiable rollout      │ Truncated          │ No optimization          │
│ Full diffusion sampling     │ Not required at    │ Yes                      │
│                             │ every update       │                          │
│ Adversariality objective    │ Optimization       │ Optional evaluation      │
│ KL regularization           │ Yes                │ No parameter update      │
│ Generator modification      │ No                 │ No                       │
│ Sampling-rule modification  │ No                 │ No                       │
└─────────────────────────────┴────────────────────┴──────────────────────────┘
```

---

### 💡 NAM in one equation-level view

```text
Original diffusion prior

        z ~ N(0, I)


                     frozen generator probe
        z  ─────────────────────────────────►  s_T
        ...
        z_adv ──────────────────────────────► x_syn
                     ...
                     frozen downstream model
        x_syn ──────────────────────────────► adversariality A(x_syn)


NAM training objective:

        maximize
            useful synthetic adversariality

        subject to
            remaining close to the native diffusion prior
...
> **NAM does not teach the diffusion model how to generate a new distribution.  
> It learns where to sample within the pretrained diffusion model's existing generative space.**
