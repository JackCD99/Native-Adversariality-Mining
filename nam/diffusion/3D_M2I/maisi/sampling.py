"""Fixed-budget deterministic Base/NAM sampling for MAISI volumes."""
...
build_maisi_loader = importlib.import_module(f"{_package}.utils.data").build_maisi_loader
MAISIConditionAugmentation = importlib.import_module(f"{_package}.utils.augmentations").MAISIConditionAugmentation
...
def _load_miner(config: Any, device: torch.device) -> ResUNet3DMiner:
    ...
    path = Path(config.maisi.checkpoints.nam_miner)
    if not path.is_file():
        raise FileNotFoundError(f"MAISI NAM miner checkpoint was not found: {path}")
...
def sample_dataset(config: Any, use_nam: bool = True) -> Path:
    ...
    settings = config.maisi.sampling
    loader = build_maisi_loader(config.dataset, "train", int(settings.batch_size), int(config.runtime.num_workers), False)
    ...
    for batch in tqdm(loader, desc=f"MAISI {method.upper()} sampling"):
        ...
        for index in range(available):
            ...
            _io.save_volume_pair(output, sample_id, images[index], targets[index], {
                ...
                "miner_checkpoint": str(config.maisi.checkpoints.nam_miner) if use_nam else None,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sample Base or NAM volumes with MAISI.")
