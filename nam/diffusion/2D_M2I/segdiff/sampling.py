"""Fixed-budget Base and NAM synthetic-set sampling for SegDiff."""
...
build_segdiff_loader = importlib.import_module(f"{_package}.utils.data").build_segdiff_loader
...
def _load_miner(config: Any, device: torch.device) -> AdversarialityMiner:
    miner = AdversarialityMiner(
        spatial_dims=2,
        in_channels=int(getattr(config.miner, "in_channels", config.diffusion.image_channels)),
        noise_channels=int(config.diffusion.image_channels),
    ...
    checkpoint_path = Path(config.segdiff.checkpoints.nam_miner)
...
def sample_dataset(config: Any, use_nam: bool = True) -> Path:
    """Generate exactly the configured budget with aligned image-mask pairs."""
    ...
    diffusion.freeze()
    settings = config.segdiff.sampling
    loader = build_segdiff_loader(
    ...
    for batch in tqdm(loader, desc=f"SegDiff {method.upper()} sampling"):
        ...
        if miner is not None:
            ...
            selected = reselect_noise(
                ...
                variance_bound=float(config.miner.variance_bound), generator=reselection_generator,
            ).sample
        ...
        for index in range(available):
            ...
            save_pair(
                output, sample_id, images[index], targets[index],
                {
                    ...
                    "miner_checkpoint": str(config.segdiff.checkpoints.nam_miner) if use_nam else None,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sample Base or NAM data with SegDiff.")
